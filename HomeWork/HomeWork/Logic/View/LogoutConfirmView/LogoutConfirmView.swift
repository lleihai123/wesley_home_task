...
struct LogoutConfirmContentView: AlertBgViewProtocol {
...
extension LogoutConfirmContentView {
    ...
    private func getContentDesc() -> some View {
        Text(AppLang("key_app_lang_user_logout_desc"))
            .font(.goMinera(16, .regular))
            .foregroundColor(textDescColor)
    }
    ...
    private func getContentBottomButtons() -> some View {
        HStack {
            ...
            Button(action: {
                dismiss()
                EventCenter.shard.dispatcherEvent(event: EventModel(eventData: "", eventType: AppEventClick.logoutConfirm, mainNeed: true))
            }, label: {
                Text(AppLang("key_app_lang_user_logout_sure"))
    ...
    private var textDescColor: Color {
        return colorScheme == .dark ? 0xFF453A.color() : 0xFF3B30.color()
    }
...
struct LogoutConfirmView: View {
    var body: some View {
        AlertBgView(contentView: LogoutConfirmContentView(), topDiff: UIDevice.statusBarHeight() - 44 + 170)
